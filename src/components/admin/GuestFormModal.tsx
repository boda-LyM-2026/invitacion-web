...
export function GuestFormModal({ grupoInicial, onCancelar, onGuardar }: GuestFormModalProps) {
  ...
  return (
    <AnimatePresence>
      <motion.div
        className="fixed inset-0 z-50 flex items-center justify-center bg-cinematic-black/80 px-4 backdrop-blur-sm"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
      >
        ...
        >
          <h2 className="font-display text-2xl font-light italic text-alabaster">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-xl border border-alabaster/20 bg-alabaster/10 px-4 py-3 font-body text-sm text-alabaster focus:border-olive focus:outline-none"
                >
                  {CATEGORIAS.map((c) => (
          ...
          <div className="mt-8 flex justify-end gap-3">
            <motion.button
              onClick={onCancelar}
              className="rounded-xl px-5 py-2.5 font-body text-sm text-alabaster/50 transition-colors hover:bg-alabaster/10 hover:text-alabaster"
            ...
            <motion.button
              ...
              className="rounded-xl bg-olive px-5 py-2.5 font-body text-sm text-alabaster shadow-glow-olive transition-all hover:bg-olive-500 disabled:opacity-50"
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
            >
